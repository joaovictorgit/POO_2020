package carro_especial;

import java.util.ArrayList;
import java.util.Scanner;

class Pessoa{
    String nome;
    int idade;
    double km;
    double preco;
    public Pessoa(String nome, int idade){
        this.nome = nome;
        this.idade = idade;
        this.km = 0;
        this.preco = 0;
        
    }
    public String toString(){
        return nome + ":" + idade;
    }
}

class Carro{
    Pessoa motorista;
    ArrayList<Pessoa> passageiros; //ISSO NAO EH UM ARRAYLIST
    public int maxPass;

    public Carro(){
        this.maxPass = 5; //se mudar aqui tem que mudar o toString
        this.passageiros = new ArrayList<>();
        for(int i = 0; i < this.maxPass; i++)
            this.passageiros.add(null);
    }

    public void in(Pessoa pessoa, int indice){
        if(indice < 0 || indice >= this.maxPass){
            System.out.println("Posicao nao existe");
            return;
        }
        if(this.passageiros.get(indice) != null){
            System.out.println("Posicao esta ocupada");
            return;    
        }
        this.passageiros.set(indice, pessoa);
    }

    public void out(String nome){
        for(int i = 0; i < this.passageiros.size(); i++)
            if(this.passageiros.get(i) != null && passageiros.get(i).nome.equals(nome)){
                this.passageiros.set(i, null);
                break;
            }
    }
    
    public void destiny(String nome, double km){
        for(int i = 0; i < this.passageiros.size(); i++)
            if(this.passageiros.get(i) != null && passageiros.get(i).nome.equals(nome)){
                this.passageiros.get(i).km = km;
                double preco = this.passageiros.get(i).km * 1.50;
                this.passageiros.get(i).preco = preco;
                
                System.out.println("Total a pagar RS " +  this.passageiros.get(i).preco);
                break;
            }else{
                System.out.println("Pessoal não está no carro");
            }
    }

    public String toString(){
        String saida = "[----|----]\n";
        saida += "[" + this.passageiros.get(0) + "|" + this.passageiros.get(1) + "]\n";
        saida += "[" + this.passageiros.get(2) + "|" + this.passageiros.get(3) + "]";
        return saida;
    }
}

public class Carro_especial {

    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in);
        Carro carro = new Carro();
        while(true){
            String line = scanner.nextLine();
            String[] ui = line.split(" ");
            if(ui[0].equals("end")){
                break;
            }else if(ui[0].equals("show")){
                System.out.println(carro);
            }else if(ui[0].equals("in")){//nome idade indice
                carro.in(new Pessoa(ui[1], Integer.parseInt(ui[2])), Integer.parseInt(ui[3]));
            }else if(ui[0].equals("init")){
                carro = new Carro();
            }else if(ui[0].equals("remove")){//nome
                carro.out(ui[1]);
            }else{
                System.out.println("fail: comando invalido");
            }
        }
        scanner.close();            
    }

}